...
from meldingen_core.location import AddressSchema, BaseAddressProviderTask, BaseAddressResolver
...
async def test_address_provider() -> None:
    ...
    resolve_address.return_value = AddressSchema(
        street="Stationsplein", house_number=39, house_number_addition="K", postal_code="1012AB", city="Amsterdam"
    )

    repository = Mock(BaseMeldingRepository)

    provide_address: BaseAddressProviderTask[Melding] = BaseAddressProviderTask(resolve_address, repository)

    await provide_address(melding, lat, lon)
    resolve_address.assert_called_once_with(lat, lon)
    repository.save.assert_awaited_once_with(melding)
